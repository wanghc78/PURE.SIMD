...
static SEXP integer_binary(ARITHOP_TYPE code, SEXP s1, SEXP s2, SEXP lcall)
{
    ...
    int temp_flag = 0;
    int si1,si2;
    __m128d NA_real_lane = _mm_set1_pd(NA_REAL);
    __m128i NA_lane = _mm_set1_epi32(NA_INTEGER);
    __m128i max_n1 = _mm_set1_epi32(n1);
    __m128i max_n2 = _mm_set1_epi32(n2);
    __m128i zero = _mm_set1_epi32(0);
    __m128i negative_one = _mm_set1_epi32(-1);
    switch (code) {
    case PLUSOP:
	// To check for most basic speed of the opperation
	/*for (i = 0; i < n; i += 4) {
		__m128i m1,m2;
		m1 = _mm_loadu_si128((__m128i *) (INTEGER(s1) + i));
		m2 = _mm_loadu_si128((__m128i *) (INTEGER(s2) + i));

		m1 = _mm_add_epi32(m1,m2);

		_mm_storeu_si128((__m128i *) (INTEGER(ans) + i),m1);
	}*/
	// Complete conversion to SIMD
	/*for (i = i1 = i2 = 0; i < n;
	     i1 = ((i1 += amount) == n1) ? 0 : i1,
	     i2 = ((i2 += amount) == n2) ? 0 : i2,
  	     i += amount)
	{
		__m128i m1,m2,i1_mask,i2_mask,NA_mask_1,NA_mask_2;
		m1 = _mm_loadu_si128((__m128i *) (INTEGER(s1) + i1));
		m2 = _mm_loadu_si128((__m128i *) (INTEGER(s2) + i2));

		__m128i a = _mm_cmpgt_epi32(m1,zero);

		NA_mask_1 = _mm_cmpeq_epi32(m1,NA_lane);
		NA_mask_2 = _mm_cmpeq_epi32(m2,NA_lane);
		NA_mask_1 = _mm_or_si128(NA_mask_1,NA_mask_2);

		i1_mask = _mm_set_epi32(i1 + 3, i1 + 2, i1 + 1, i1);
		i2_mask = _mm_set_epi32(i2 + 3, i2 + 2, i2 + 1, i2);
		i1_mask = _mm_cmplt_epi32(i1_mask,max_n1);
		i2_mask = _mm_cmplt_epi32(i2_mask,max_n2);
		i1_mask = _mm_and_si128(i1_mask,i2_mask);

		m1 = _mm_add_epi32(m1,m2);
		NA_mask_2 = _mm_cmpeq_epi32(m1,NA_lane);
		NA_mask_1 = _mm_or_si128(NA_mask_1,NA_mask_2);
		__m128i b = _mm_cmplt_epi32(m1,m2);
		__m128i c = _mm_cmplt_epi32(m2,m1);
		a = _mm_blendv_epi8(c,b,a);
		temp_flag = !_mm_test_all_zeros(a,i1_mask);
		NA_mask_1 = _mm_or_si128(NA_mask_1,a);
		m1 = _mm_blendv_epi8(m1,NA_lane,NA_mask_1);

		_mm_maskmoveu_si128(m1,i1_mask,(char *)(INTEGER(ans) + i));
		//_mm_storeu_si128((__m128i *) (INTEGER(ans) + i),m1);

		amount = 0;
		amount += 1 & _mm_extract_epi32(i1_mask,0);
		amount += 1 & _mm_extract_epi32(i1_mask,1);
		amount += 1 & _mm_extract_epi32(i1_mask,2);
		amount += 1 & _mm_extract_epi32(i1_mask,3);
	}
	if (temp_flag) {
	...
	if (n1 % 4 == 0 && n2 % 4 == 0) {
		for (i = i1 = i2 = 0; i < n;
		     ...
		     i2 = ((i2 += 4) == n2) ? 0 : i2,
	  	     i += 4)
		{
			__m128i m1,m2,val,NA_mask_1,NA_mask_2;
			m1 = _mm_loadu_si128((__m128i *) (INTEGER(s1) + i1));
			m2 = _mm_loadu_si128((__m128i *) (INTEGER(s2) + i2));
			...
			__m128i c = _mm_cmplt_epi32(m2,m1);
			a = _mm_blendv_epi8(c,b,a);
			temp_flag = !_mm_test_all_zeros(a,negative_one);
			NA_mask_1 = _mm_or_si128(NA_mask_1,a);*/
			//val = _mm_blendv_epi8(val,NA_lane,NA_mask_1);

			_mm_storeu_si128((__m128i *) (INTEGER(ans) + i),val);
		}
		if (temp_flag) {
			warningcall(lcall, INTEGER_OVERFLOW_WARNING);
		}
...
//	    		if ((i+1) % NINTERRUPT == 0) R_CheckUserInterrupt();
  	     ...
  	     i += amount)
	{
		...
		__m128i a = _mm_cmplt_epi32(m1,zero);
		__m128i b = _mm_cmplt_epi32(m2,zero);
		...
		m1 = _mm_sub_epi32(m1,m2);
		NA_mask_2 = _mm_cmpeq_epi32(m1,NA_lane);
		NA_mask_1 = _mm_or_si128(NA_mask_1,NA_mask_2);
		__m128i c = _mm_cmplt_epi32(m1,zero);
		b = _mm_xor_si128(_mm_andnot_si128(a,b),_mm_or_si128(a,b));
		c = _mm_xor_si128(c,a);
		a = _mm_and_si128(c,b);
		temp_flag = !_mm_test_all_zeros(a,i1_mask);
		NA_mask_1 = _mm_or_si128(NA_mask_1,a);
  	     ...
  	     i += amount)
	{
		...
		__m128 check = _mm_mul_ps(_mm_cvtepi32_ps(m1),_mm_cvtepi32_ps(m2));
		m1 = _mm_mullo_epi32(m1,m2);
		NA_mask_2 = _mm_cmpeq_epi32(m1,NA_lane);
		NA_mask_1 = _mm_or_si128(NA_mask_1,NA_mask_2);
		__m128i a = _mm_castps_si128(_mm_cmpneq_ps(_mm_cvtepi32_ps(m1),check));
		temp_flag = !_mm_test_all_zeros(a,i1_mask);
		NA_mask_1 = _mm_or_si128(NA_mask_1,a);
		m1 = _mm_blendv_epi8(m1,NA_lane,NA_mask_1);
  	     ...
  	     i += amount)
	{
		...
		i1_mask = _mm_set_epi32(1000000000, 1000000000, i1 + 1, i1); // should use max_int
		i2_mask = _mm_set_epi32(1000000000, 1000000000, i2 + 1, i2); // should use max_int
		i1_mask = _mm_cmplt_epi32(i1_mask,max_n1);
		i2_mask = _mm_cmplt_epi32(i2_mask,max_n2);
		i1_mask = _mm_and_si128(i1_mask,i2_mask);

		__m128d result = _mm_div_pd(_mm_cvtepi32_pd(m1),_mm_cvtepi32_pd(m2));
		result = _mm_blendv_pd(result,NA_real_lane,_mm_cvtepi32_pd(NA_mask_1));
		
		_mm_storeu_pd((REAL(ans) + i),result);

		//_mm_maskmoveu_si128(_mm_castpd_si128(result),i1_mask,(char *)(REAL(ans) + i));
		//_mm_storeu_si128((__m128i *) (INTEGER(ans) + i),m1);
	...
	if(TYPEOF(s1) == REALSXP && TYPEOF(s2) == REALSXP) {
		...
		if (tmp == NA_REAL) {
			...
			REAL(ans)[i] = NA_REAL;
                ...
                for (i = 0; i < n; i++) {
                    if (REAL(s1)[i] == NA_REAL || REAL(s2)[i] == NA_REAL) {
                        REAL(ans)[i] = NA_REAL;
                    }
		    else {
			REAL(ans)[i] = REAL(s1)[i] + REAL(s2)[i];
                ...
                mod_iterate(n1, n2, i1, i2) {
//		    if ((i+1) % NINTERRUPT == 0) R_CheckUserInterrupt();
                    if (REAL(s1)[i1] == NA_REAL || REAL(s2)[i2] == NA_REAL) {
                        REAL(ans)[i] = NA_REAL;
                    }
		    else {
	   ...
	   mod_iterate(n1, n2, i1, i2) {
//	       if ((i+1) % NINTERRUPT == 0) R_CheckUserInterrupt();
	       if (INTEGER(s1)[i1] == NA_INTEGER || REAL(s2)[i2] == NA_REAL) {
                   REAL(ans)[i] = NA_REAL;
               }
	       else {
	   ...
	   mod_iterate(n1, n2, i1, i2) {
//	       if ((i+1) % NINTERRUPT == 0) R_CheckUserInterrupt();
	       if (INTEGER(s2)[i2] == NA_INTEGER || REAL(s1)[i1] == NA_REAL) {
                   REAL(ans)[i] = NA_REAL;
               }
	       else {
