...
static SEXP integer_binary(ARITHOP_TYPE code, SEXP s1, SEXP s2, SEXP lcall)
{
    ...
    case PLUSOP:
	/*if (n1 == 1) {
		__m128i m1 = _mm_set1_epi32(INTEGER(s1)[0]);
		__m128i NA_mask_1 = _mm_cmpeq_epi32(m1,NA_lane);
		for (i = 0; i < n2; i += 4) {
			__m128i m2,val,NA_mask_2;
			m2 = _mm_loadu_si128((__m128i *) (INTEGER(s2) + i));

			//__m128i a = _mm_cmpgt_epi32(m1,zero);

			NA_mask_2 = _mm_cmpeq_epi32(m2,NA_lane);
			NA_mask_2 = _mm_or_si128(NA_mask_1,NA_mask_2);
			...
			//val = _mm_blendv_epi8(val,NA_lane,NA_mask_2);

			//_mm_storeu_si128((__m128i *) (INTEGER(ans) + i),val);
		//}
	//}
	/*else if (n2 == 1) {
		__m128i m1 = _mm_set1_epi32(INTEGER(s2)[0]);
		__m128i NA_mask_1 = _mm_cmpeq_epi32(m1,NA_lane);
		for (i = 0; i < n1; i += 4) {
			__m128i m2,val,NA_mask_2;
			m2 = _mm_loadu_si128((__m128i *) (INTEGER(s1) + i));
